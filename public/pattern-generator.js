(function () {
  ...
  function scallop(w, h) {
    ...
    for (var x = -tw; x < w + tw * 2; x += tw) {
      ctx.beginPath(); ctx.arc(x, cy, r, Math.PI, 0); ctx.stroke();
      ctx.beginPath(); ctx.arc(x + tw / 2, cy, r, 0, Math.PI); ctx.stroke();
      ctx.beginPath(); ctx.arc(x, cy, ir, Math.PI, 0); ctx.stroke();
      ctx.beginPath(); ctx.arc(x + tw / 2, cy, ir, 0, Math.PI); ctx.stroke();
      dot(x, cy, 1.5);
    }
  }
  ...
  function diamonds(w, h) {
    ...
    for (var x = 0; x < w + tw; x += tw) {
      ctx.beginPath();
      ctx.moveTo(x, cy - dy); ctx.lineTo(x + dx, cy); ctx.lineTo(x, cy + dy); ctx.lineTo(x - dx, cy);
      ctx.closePath(); ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(x, cy - dy * 0.5); ctx.lineTo(x + dx * 0.5, cy); ctx.lineTo(x, cy + dy * 0.5); ctx.lineTo(x - dx * 0.5, cy);
      ctx.closePath(); ctx.stroke();
      dot(x, cy, 1.8); dot(x, cy - dy, 1); dot(x, cy + dy, 1);
      ctx.beginPath(); ctx.moveTo(x + dx, cy); ctx.lineTo(x + tw - dx, cy); ctx.stroke();
    }
  }
  ...
  function chainCircles(w, h) {
    ...
    for (var x = 0; x < w + sp; x += sp) {
      ctx.beginPath(); ctx.arc(x, cy - r * 0.2, r, 0, Math.PI * 2); ctx.stroke();
      ctx.beginPath(); ctx.arc(x + sp / 2, cy + r * 0.2, r, 0, Math.PI * 2); ctx.stroke();
      ctx.beginPath(); ctx.arc(x, cy - r * 0.2, r * 0.3, 0, Math.PI * 2); ctx.stroke();
      ctx.beginPath(); ctx.arc(x + sp / 2, cy + r * 0.2, r * 0.3, 0, Math.PI * 2); ctx.stroke();
    }
  }
  ...
  function crossDots(w, h) {
    ...
    for (var x = tw / 2; x < w + tw; x += tw) {
      ...
      ctx.stroke();
      dot(x, cy, 2);
      dot(x - s, cy - s, 0.8); dot(x + s, cy - s, 0.8);
      dot(x - s, cy + s, 0.8); dot(x + s, cy + s, 0.8);
    }
  }
  ...
  function floralScroll(w, h) {
    ...
    for (var x = -tw; x < w + tw * 2; x += tw) {
      ...
      ctx.stroke();
      ctx.beginPath(); ctx.arc(x + tw * 0.3, cy - a * 0.7, 3.5, 0, Math.PI * 2); ctx.stroke();
      dot(x + tw * 0.3, cy - a * 0.7, 1.5);
      ctx.beginPath(); ctx.arc(x + tw * 0.7, cy + a * 0.7, 3.5, 0, Math.PI * 2); ctx.stroke();
